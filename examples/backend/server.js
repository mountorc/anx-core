...
const { handleTapSet, handleTriggerSet, processTriggerCardKey, handleTapSetRequestSet, handleTapSetRequestSetAsync } = require('../../core/utils/trigger-and-tap.js');
...
app.post('/api/getNodes', async (req, res) => {
  try {
...
app.post('/api/getConfig', async (req, res) => {
  try {
    const { uuid_page, url_tile, uuid_tile } = req.body;
    
    let config = null;
    
    if (uuid_page) {
      const page = getPageWithNodes(uuid_page);
      if (page && page.nodes && page.nodes.config) {
        config = page.nodes.config;
        console.log(`[getConfig] Loaded config from uuid_page: ${uuid_page}`);
      }
    }
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (result.config) {
            ...
            console.log(`[getConfig] Loaded config from url_tile: ${url_tile}`);
          }
        }
      } catch (error) {
        console.error(`[getConfig] Error loading config from url_tile:`, error);
      }
    }
    ...
    if (!config && uuid_tile) {
      ...
      if (hubEntry && hubEntry.anxContent) {
        ...
        console.log(`[getConfig] Loaded config from uuid_tile: ${uuid_tile}`);
      }
    }
  ...
  res.status(400).json({ success: false, error: 'Error getting config' });
}
});

app.post('/api/getMarkup', async (req, res) => {
  try {
    ...
    if (!resolvedUuidPage) {
      ...
      if (visitorPages.length > 0) {
        ...
        if (filteredPages.length > 0) {
          ...
          console.log(`[getMarkup] Found existing page for visitor: ${resolvedUuidPage}`);
        }
      }

      if (!resolvedUuidPage) {
        console.log(`[getMarkup] No existing page found, creating new page...`);
        ...
        if (url_tile) {
          try {
            ...
            console.error(`[getMarkup] Error loading config from url_tile:`, error);
          }
        } else if (uuid_tile) {
          const hubEntry = hubAnxMap.get(uuid_tile);
          if (hubEntry && hubEntry.anxContent) {
        ...
        console.log(`[getMarkup] Created new page: ${resolvedUuidPage}`);
      }
    }

    const result = await getPageMarkup(resolvedUuidPage);
...
app.post('/api/trigger-card-key', async (req, res) => {
  try {
    ...
    const result = await processTriggerCardKey({ cardKey, tapSet, triggerSet, data });
    res.json(result);
  } catch (error) {
